...
var visitor = function (node) {
	...
	if (
		...
		if (
			array &&
			((_a = array.elements) === null || _a === void 0
				? void 0
				: _a.some(function (el) {
						return !isStringLiteral(el);
					}))
		) {
		...
		for (var _i = 0, _d = object.properties; _i < _d.length; _i++) {
			...
			properties.push(
				createGetter({ name: name_1, initializer: initializer, stringLiteral: stringLiteral }),
			);
			if (setter) {
...
var createGetter = function (_a) {
	var name = _a.name,
		...
		stringLiteral = _a.stringLiteral;
...
var createSetter = function (_a) {
	var name = _a.name,
		initializer = _a.initializer;
	// Prevent shadowing
	var local = ["v", "_v", "__v"].find(function (v) {
		return !new Set([name, initializer]).has(v);
	});
