...
<p>Feel free to open the component files and modify the styles to suit your design. If you want to tweak the logic in a reusable way then have a look at the corresponding headless component.</p>
<hr>
<h2>Headless components</h2>
<p>Every UI component has a corresponding headless component containing only the sharable logic, attributes and behavior.</p>
<p>When you instanciate a headless component you can use it with the <code>choco</code> action. The preprocessor takes care of spreading the attributes and manages actions for you.</p>
...
`} lang="svelte"></Highlighter>
<p>In the above example there is no clash between the toggle’s inner <code>click</code> event listener and the one declared on the button. The <code>ChocoBase</code> class and all headless components pass their behavior through an action, avoiding clashes with other declarative listeners.</p>
<hr>
<h2>Mixins</h2>
...
`} lang="ts"></Highlighter>
<p>You see how we could very easily adapt this to create a headless switch component (by toggling <code>aria-checked</code>). Many things in a UI are togglable so this is a powerful abstraction and a good starting point. We could build from there to have a disclosure component (toggling <code>aria-expanded</code>), a hoverable (by toggling on during <code>mouseenter</code> and off during <code>mouseleave</code> etc). See the corresponding mixins for more on these low level primitives.</p>
<p>Also notice how readable and short the code is.</p>
<hr>
<h2>And more…</h2>
<p>There is also a Vite plugin, which combined to some TypeScript choco spells will help you manage two-way bindings across boundaries in a type-safe way. More about this topic <a href="/guide/plugin">here</a></p>